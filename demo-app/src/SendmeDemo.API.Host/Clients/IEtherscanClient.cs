...
{
    [Get("/api?module=account&action=tokentx&contractaddress={contractAddress}&address={address}&page=1&offset=20&sort=desc&apikey={accessToken}")]
    public Task<EtherscanResponse> GetTransactionsAsync(string contractAddress, string address, string accessToken);
    
    [Get("/api?module=account&action=tokentx&contractaddress={contractAddress}&page=1&offset=20&sort=desc&apikey={accessToken}")]
